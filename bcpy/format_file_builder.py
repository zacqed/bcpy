class FormatFile:
    ...
    def _get_field_terminators(cls, flat_file):
        ...
        terminators.extend([qualifier_delimiter_combo for _ in range(len(flat_file.columns) - 1)])
    ...
    def build_format_file(cls, data_object):
        ...
        if data_object.qualifier:
            format_file += (
                f'1 SQLCHAR 0 0 "{terminators[0]}" 0 ' f"ignored_line_start_qualifier SQL_Latin1_General_CP1_CI_AS\n"
            )
        ...
        for column_index, terminator in enumerate(terminators, 1):
            format_file += (
                f"{format_file_row_index} SQLCHAR 0 0 "
